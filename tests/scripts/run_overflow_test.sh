...
expected_left_req=10
expected_left_resp=10
expected_flush=2
...
if [[ $left_req -eq $expected_left_req && $left_resp -eq $expected_left_resp && $flush_req -eq $expected_flush && $flush_resp -eq $expected_flush ]]; then
  ...
  echo "  leftover request lines: $left_req expected $expected_left_req" >&2
  echo "  leftover response lines: $left_resp expected $expected_left_resp" >&2
