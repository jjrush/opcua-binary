# @TEST-EXEC: zeek -C -r ${TRACES}/opcua_with-gap_with-handshake.pcap %INPUT
...
# @TEST-EXEC: zeek-cut -n opcua_link_id status_code_link_id request_header_link_id response_header_link_id < opcua_binary.log > opcua_binary.tmp && mv opcua_binary.tmp opcua_binary.log
...
#
# @TEST-DOC: Test OPCUA-binary analyzer with a trace file that contains a ga in the transmitted data followed by a second ACK, HEL handshake.
